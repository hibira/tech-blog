...
`env` タグを併用することで、インシデントの優先度も自動的にマッピングされます。

| `env` タグの値 | インシデント優先度 |
|---|---|
| `prod` / `production` | CRITICAL |
| `uat` | MEDIUM |
| `dev` | LOW |
| 未設定 | LOW |

本番環境のアラームは自動的に CRITICAL として扱われるため、DevOps Agent 側での優先度設定も不要です。

この Priority は Webhook ペイロードの `priority` フィールドとして DevOps Agent に送信されます。DevOps Agent はこの値をインシデントのトリアージに利用し、CRITICAL のインシデントはダッシュボード上で上位に表示されます。複数のアラームが同時に発報した場合でも、本番環境のインシデントが優先的に目に入るため、対応の優先順位を判断しやすくなります。

また、Priority のマッピングロジックは Lambda 関数内の `mapPriority` 関数で定義されているため、チームの運用に合わせてカスタマイズも可能です。たとえば `staging` 環境を追加したい場合は、以下のように関数を修正するだけです。

```javascript
function mapPriority(envRaw) {
  const env = (envRaw || "").toLowerCase();
  if (env === "prod" || env === "production") return "CRITICAL";
  if (env === "staging") return "HIGH";      // 追加
...
aws cloudwatch tag-resource \
  ...
  --tags Key=auto_investigate,Value=true Key=env,Value=prod
```

<!-- 📸 画像5: CloudWatch コンソールのアラーム詳細画面 → Tags タブ。auto_investigate=true と env=prod のタグが付与されている状態 -->
